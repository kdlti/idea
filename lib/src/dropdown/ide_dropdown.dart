...
export 'ide_dropdown.dart';

// models
part 'models/ide_dropdown_decoration.dart';
part 'models/ide_dropdown_list_filter.dart';
...
enum DropdownState { closed, opened }
...
class IdeDropdown<T> extends StatefulWidget {
  ...
  /// Called when the list of items of the [IdeDropdown] should change.
  final Function(List<T>)? onListChanged;

  /// Hide the selected item from the [items] list.
  final bool excludeSelected;

  /// Can close [IdeDropdown] overlay by tapping outside.
  /// Here "outside" covers the entire screen.
  final bool canCloseOutsideBounds;

  /// Hide the header field when [IdeDropdown] overlay opened/expanded.
  ...
  /// Padding for [IdeDropdown] header (closed state).
  final EdgeInsets? closedHeaderPadding;

  /// Padding for [IdeDropdown] header (opened/expanded state).
  final EdgeInsets? expandedHeaderPadding;

  /// Padding for [IdeDropdown] items list.
  final EdgeInsets? itemsListPadding;

  /// Padding for [IdeDropdown] each list item.
  final EdgeInsets? listItemPadding;

  /// Widget to display while search request loading.
  final Widget? searchRequestLoadingIndicator;

  /// [IdeDropdown] opened/expanded area height.
  ...
  /// The [headerBuilder] that will be used to build [IdeDropdown] header field.
  final _HeaderBuilder<T>? headerBuilder;

  /// The [hintBuilder] that will be used to build [IdeDropdown] hint of header field.
  ...
  const IdeDropdown.searchRequest({
    super.key,
  ...
  const IdeDropdown.multiSelectSearchRequest({
    super.key,
    required this.futureRequest,
    required this.onListChanged,
    this.futureRequestDelay,
  ...
  @override
  State<IdeDropdown<T>> createState() => _IdeDropdownState<T>();
}

class _IdeDropdownState<T> extends State<IdeDropdown<T>> {
  ...
  @override
  void didUpdateWidget(covariant IdeDropdown<T> oldWidget) {
        ...
        return _OverlayBuilder(
          ...
          overlay: (size, hideCallback) {
            return _DropdownOverlay<T>(
              ...
              searchHintText: widget.searchHintText ?? 'Procurar',
