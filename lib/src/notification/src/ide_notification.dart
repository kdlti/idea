...
import 'package:idea/src/notification/src/resources/constants.dart';
import 'package:idea/src/notification/src/resources/extensions.dart';
import 'package:idea/src/notification/src/resources/stacked_options.dart';
import 'package:idea/src/notification/src/widgets/animated_progress_bar.dart';
import 'package:idea/src/notification/src/widgets/overlay_manager.dart';
import 'package:idea/src/notification/src/widgets/toast_content.dart';

// ignore: must_be_immutable
class IdeNotification extends StatefulWidget {
  IdeNotification({
    super.key,
    this.title,
    ...
    this.closeButton,
    this.stackedOptions,
    this.notificationHorizontalMargin = 0,
    this.notificationVerticalMargin = 0,
  ...
  IdeNotification.success({
    ...
    this.background = Colors.white,
    this.closeButton,
    this.stackedOptions = const StackedOptions(),
    this.notificationHorizontalMargin = 0,
    this.notificationVerticalMargin = 20,
    this.toastDuration = const Duration(milliseconds: 3000),
    this.displayCloseButton = true,
    this.onCloseButtonPressed,
    this.onProgressFinished,
  ...
  IdeNotification.error({
    ...
    this.closeButton,
    this.stackedOptions,
    this.notificationHorizontalMargin = 20,
    this.notificationVerticalMargin = 20,
    this.toastDuration = const Duration(milliseconds: 3000),
    this.displayCloseButton = true,
    this.onCloseButtonPressed,
  ...
  /// The margin horizontal between the notification and the edge of the screen
  final double notificationHorizontalMargin;

  /// The margin vertical between the notification and the edge of the screen
  final double notificationVerticalMargin;
  ...
  double alignmentToLeftPos(BuildContext context) {
    if (position.x == 1) {
      return MediaQuery.of(context).size.width - mainContainerWidth(context) - notificationHorizontalMargin;
    } else if (position.x == -1) {
      return notificationHorizontalMargin;
    } else {
  ...
  double alignmentToTopPos(BuildContext context) {
    if (position.y == 1) {
      return MediaQuery.of(context).size.height - mainContainerHeight(context) - notificationVerticalMargin;
    } else if (position.y == -1) {
      return notificationVerticalMargin;
    } else {
  ...
  @override
  IdeNotificationState createState() => IdeNotificationState();
}

class IdeNotificationState extends State<IdeNotification> with SingleTickerProviderStateMixin {
  @override
  void initState() {
    super.initState();
